...
export default class MasterPage extends Vue {
   ...
   * zero. This function that emits the count down values is in the master client. Every secinds emits a
   * new value but stops if the gamestate is STORE_GAME_STATUS_MASTER_EVALUATING_ANSWER that means that
   * a user have sent an answer and the master is evaluating it. If the counter reaches zero, the answer
   * attempt is counted as wrong.
   */
  ...
  /**
   * This function creates the object to send to the players to tell if the answer was right or wrong.
   */
  ...
  /**
   * Clear the input fields of the question and the answers.
   */
  clearQuestionFields() {
    this.question = null;
    this.answerOne = null;
